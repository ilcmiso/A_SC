Public Class VBR_PI
    ...
    Private Sub WriteExcel()
        ...
        For row = 0 To dt.Rows.Count - 1
            ...
            xidx = 0
            ' 大項目毎(リストボックスの項目数)
            For cNum = 0 To dt.Columns.Count - 1 - 1  ' dtから顧客番号C01以降のデータを取得するから、更に-1
                If Not OwnerForm.LB_SELITEM.GetSelected(cNum) Then Continue For    ' リストボックス未選択の項目は出力しない
                words = dt.Rows(row).Item(cNum + 1).ToString.Split("`")
            ...
            If foundData Then
                ...
                End If
                'CellReport1.Pos(1, yidx + CCNT).Value = dt.Rows(row).Item(1)        
                yidx += 1
            End If
        Next
    End Sub
